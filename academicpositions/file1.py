...
driver.get('https://academicpositions.com/find-jobs/all-in-all-by-all-in-all/all/1')
source = driver.page_source
soup = BeautifulSoup(source, 'html.parser')
txt_str = soup.find("h3", class_="section-title section-title-small--inline").text
x = re.findall("\d", txt_str)
y = ""
for i in x:
  y = y+i
a = int(y)/20
...
# mycursor.execute("CREATE TABLE data_academicpositions(ID INT AUTO_INCREMENT PRIMARY KEY, University VARCHAR(255), Address varchar(255), Country varchar(255), Link varchar(255), Date_Time varchar(255))")
sql = "INSERT INTO tbl_academicpositions(ID, University, Address, Country, Link, Date_Time) VALUES (%s, %s, %s, %s, %s, %s)"
...
sql = "DELETE n2 FROM tbl_academicpositions n1, tbl_academicpositions n2 WHERE n1.University = n2.University and n1.ID < n2.ID"
mycursor.execute(sql)
mycursor.execute("alter table tbl_academicpositions drop ID")
mycursor.execute("ALTER TABLE tbl_academicpositions ADD COLUMN ID INT AUTO_INCREMENT PRIMARY KEY")
mydb.commit()
